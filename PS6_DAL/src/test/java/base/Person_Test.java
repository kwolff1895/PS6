...
public class Person_Test {
	...
	public static void setUpBeforeClass() throws Exception {
		...
		Date dDate = null;
		try {
			dDate = new SimpleDateFormat("yyyy-MM-dd").parse("1995-06-18");
		} catch (ParseException e) {
			e.printStackTrace();
		}
		
		per1 = new PersonDomainModel();
		per1.setFirstName("Kim");
		per1.setLastName("Wolff");
		per1.setBirthday(dDate);
		per1.setCity("Tampa");
		per1.setPostalCode(19010);
		per1.setStreet("3 Countryside Road");
	}
	...
	{		
		PersonDomainModel per;
		final String C_LASTNAME = "Fischer";
